...
class OurTrainer_l2l_mutidataset(Trainer):

    from transformers.trainer_pt_utils import _get_learning_rate, log_metrics, metrics_format, save_metrics, save_state

    # def _set_meta_model(self, meta_model_positive, meta_model_negative, meta_model_estimate):
    #     self.meta_model_positive = meta_model_positive
    #     self.meta_model_negative = meta_model_negative
    #     self.meta_model_estimate = meta_model_estimate
    def __init__(self, *args, task_name_list = None, train_dataset_list = None, data_collator_list = None, **kwargs):
        super().__init__(*args, **kwargs)
        self.task_name_list = task_name_list
        self.train_dataset_list = train_dataset_list
        self.data_collator_list = data_collator_list
        self.task_dict = {}
        for i, task_name in enumerate(self.task_name_list):
            self.task_dict[task_name] = {"train_dataset": train_dataset_list[i], "data_collator": data_collator_list[i]}

    def my_get_train_sampler(self, train_dataset) -> Optional[torch.utils.data.Sampler]:
        if train_dataset is None or not has_length(train_dataset):
            return None

        # Build the sampler.
        if self.args.group_by_length: #F
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        for i, task_name in enumerate(self.task_name_list):
            train_dataset = self.train_dataset_list[i]
            data_collator = self.data_collator_list[i]
            if is_datasets_available() and isinstance(train_dataset, datasets.Dataset):
            ...
            if isinstance(train_dataset, torch.utils.data.IterableDataset):#F
                ...
                self.task_dict[task_name]["train_dataloader"] = train_dataloader
            else:

                train_sampler = self.my_get_train_sampler(train_dataset)
    ...
    def _inner_training_loop(
        ...
        self.train_dataloader_list = self.get_train_dataloader()
        ...
        len_dataloader = None
        if has_length(self.train_dataloader_list[0]):#T
            len_dataloader = len(self.train_dataloader_list[0]) #4
            num_update_steps_per_epoch = len_dataloader // args.gradient_accumulation_steps * len(self.train_dataloader_list)
            num_update_steps_per_epoch = max(num_update_steps_per_epoch, 1)                                    
            num_examples = self.num_examples(self.train_dataloader_list[0]) * len(self.train_dataloader_list)  
            if args.max_steps > 0: ###args.max_steps=-1
                ...
                num_train_samples = self.num_examples(self.train_dataloader_list[0]) * len(self.train_dataloader_list) * args.num_train_epochs 
        ...
        # Check if continuing training from a checkpoint #F
        ...
        self.callback_handler.train_dataloader = self.train_dataloader_list[0]
        ...
        tr_loss_dict ={ task_name: torch.tensor(0.0).to(args.device) for task_name in self.task_name_list}
        # _total_loss_scalar is updated everytime .item() has to be called on tr_loss and stores the sum of all losses
        self._total_loss_scalar_dict ={ task_name: 0.0 for task_name in self.task_name_list}
        ...
        for task_name in self.task_name_list:
            self.perturbation_variance[task_name] = {}
        self.MLP_perturb = {}
        for name, param in model.named_parameters():
            ...
            if param.requires_grad:
                ...
                if 'bias' in name or 'norm' in name:
                    for task_name in self.task_name_list:
                        self.perturbation_variance[task_name][name] = torch.ones_like(param[0:1], device=param.data.device, dtype=param.data.dtype)
                else:
                    for task_name in self.task_name_list:
                        self.perturbation_variance[task_name][name] = torch.ones_like(param[0:1, 0:1], device=param.data.device, dtype=param.data.dtype)
        ...
        self.steps_per_epoch = len(self.train_dataloader_list[0]) 
        ...
        #self.LLM_optimizer = Adam(self.model_params, lr=self.args.lr_llm)
        
        self.LLM_lr_scheduler = LambdaLR(
                    self.LLM_optimizer, lr_lambda=lambda step: linear_decay_lr(step, self.restart_total_steps * len(self.task_name_list)))
        ...
        if self.args.model_name == "meta-llama/Llama-3.2-1B" or self.args.model_name == "facebook/opt-125m": 
            self.meta_model_estimate.lm_head.weight = self.meta_model_estimate.model.embed_tokens.weight
        if self.args.model_name == "facebook/opt-30b":
        ...
        self.true_gradient = {}
        for task_name in self.task_name_list:
            self.log_original_loss[task_name] = []
            self.log_meta_loss[task_name] = []
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            steps_in_epoch = (
                ...
                if self.train_dataloader_list is not None
            ...
            epoch_step = -1
            ######## Skip reading
            task_iters = {name: iter(self.task_dict[name]["train_dataloader"]) for name in self.task_name_list}
            self.history_loss = {}
            for epoch_step in range(self.steps_per_epoch):
                self.epoch_step = epoch_step
                import random
                task_list = self.task_name_list.copy() 
                if self.args.shuffle:
                    random.shuffle(task_list)            
                for i, task_name in enumerate(task_list):
                    if self.epoch_step % 4 == 1 and task_name in {"Copa"}:
                        continue
                    self.current_task = task_name
                    self.global_step_in_one_epoch = epoch_step * len(self.task_name_list) + i
                    inputs = next(task_iters[task_name], None)
                    if inputs is None:
                        task_iters[task_name] = iter(self.task_dict[task_name]["train_dataloader"])
                        inputs = next(task_iters[task_name])

                    ######## Skip reading
                    if steps_trained_in_current_epoch > 0: #F
                        ...
                        if steps_trained_in_current_epoch == 0:
                            self._load_rng_state(resume_from_checkpoint)
                        continue
                    elif steps_trained_progress_bar is not None: #F
                        steps_trained_progress_bar.close()
                        steps_trained_progress_bar = None
                    if epoch_step % args.gradient_accumulation_steps == 0:
                    ...
                    for i in range(1): # main model update nums
                        for (name, param) in model.named_parameters():
                            ...
                            param_obj_estimate._parameters[param_type] = torch.nn.Parameter(param.data)
                        ...
                        if self.epoch_step == 0:
                            self.history_loss[self.current_task] = torch.zeros(2, dtype=self.dtype).detach()
                        ...
                        ########smooth loss
                        self.log_original_loss[self.current_task].append(self.loss_positive.item())
                        self.log_meta_loss[self.current_task].append(self.loss_estimate.item())
                        if self.epoch_step % 10 == 0:
                            lr = self.LLM_optimizer.param_groups[0]["lr"]
                            logger.info(f" {self.current_task}: Adamw_LR = {lr}")
                            original_loss = round(np.mean(self.log_original_loss[self.current_task]), 4)
                            meta_loss = round(np.mean(self.log_meta_loss[self.current_task]), 4)
                            logger.info(f" {self.current_task}: original_loss = {original_loss}")
                            logger.info(f"{self.current_task}: meta_loss = {meta_loss}")
                            self.log_original_loss[self.current_task] = []
                            self.log_meta_loss[self.current_task] = []
                            
                        #update the history loss
                        self.history_loss[self.current_task] = torch.cat((self.loss_positive.detach().unsqueeze(0), 
                    ...
                    if True:  # add for loop for multi dataset
                        if (((self.epoch_step + 1) % args.gradient_accumulation_steps != 0)
                            ...
                            tr_loss_step = self.training_step(model, inputs)
                            #tr_loss_step# = torch.tensor(0.0, dtype=self.data_type, device=self.data_device)
                    ...
                    if (
                        ...
                        tr_loss_dict[self.current_task] += tr_loss_dict[self.current_task] / (1 + (self.state.global_step - self._globalstep_last_logged)/len(self.task_name_list))
                    else:
                        tr_loss_dict[self.current_task] += tr_loss_step

                    # ##### representing Line 17 in algorithm 2 of Latex 
                    self.current_flos += float(self.floating_point_ops(inputs))
                    if (self.epoch_step + 1) % args.gradient_accumulation_steps == 0 or (
                        # last step in epoch but step is always smaller than gradient_accumulation_steps
                        steps_in_epoch <= args.gradient_accumulation_steps
                        and (self.epoch_step + 1) == steps_in_epoch): #T
                        ...
                        self.state.epoch = epoch + (self.epoch_step + 1) / steps_in_epoch
                        self.control = self.callback_handler.on_step_end(args, self.state, self.control)
                    
                        self._maybe_log_save_evaluate(self.loss_positive, tr_loss_dict[self.current_task], model, trial, epoch, ignore_keys_for_eval)
        ...
        self._total_loss_scalar_dict[self.current_task] += tr_loss_dict[self.current_task].item()
        train_loss = self._total_loss_scalar_dict[self.current_task] / self.state.global_step/len(self.task_name_list)
        ...
        for (name, param) in self.named_parameters_to_optim:
            ...
            if 'bias' in name or 'norm' in name:
                with torch.no_grad():
                    ...
                    loss_state = self.history_loss[self.current_task].to(param.device)
                    model_state_temp = torch.cat((param_mean, param_var, loss_state, self.perturbation_variance[self.current_task][name].detach()))
                self.perturbation_variance[self.current_task][name] = torch.abs(self.MLP_perturb[name](model_state_temp.detach()))
                if self.args.need_normalization:
                    self.normalization_norm += param.numel() / 1e6 * self.perturbation_variance[self.current_task][name]**2
            else:
                with torch.no_grad():
                    param_mean = torch.mean(param).unsqueeze(0).unsqueeze(1)
                    param_var = torch.var(param).unsqueeze(0).unsqueeze(1)
                    loss_state = self.history_loss[self.current_task].unsqueeze(0).to(param.device)
                    model_state_temp = torch.cat((param_mean, param_var, loss_state, self.perturbation_variance[self.current_task][name].detach()), dim=1)
                self.perturbation_variance[self.current_task][name] = torch.abs(self.MLP_perturb[name](model_state_temp.detach()))
                if self.args.need_normalization:
                    if 'embed' in name:
                        self.normalization_norm += 15 * self.args.train_batch_size * self.args.world_size / 1e6 * param.shape[1] * self.perturbation_variance[self.current_task][name]**2
                    else:
                        self.normalization_norm +=  param.shape[0] / 1e6 * param.shape[1] * self.perturbation_variance[self.current_task][name]**2
    ...
    def use_estimate_sample_to_perturb(self, model):
        torch.manual_seed(self.random_seed)
        if self.args.need_normalization == True:
            self.normalization_factor = torch.sqrt(self.valid_gradient_num_param) / torch.sqrt(self.normalization_norm)
        for (name, param), (_, meta_positive_param), (_, meta_negative_param) in zip(model.named_parameters(),
            ...
            perturbation_of_estimate_sample = (u * self.args.zo_eps).detach() * self.perturbation_variance[self.current_task][name]
            if self.args.need_normalization:
    ...
    def use_estimate_gradient_to_perturb(self, model):
        ...
        for (name, param), (_, meta_estimate_param) in zip(model.named_parameters(),
            ...
            u = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
            param_obj_estimate, param_type = self.get_parameter_value(self.meta_model_estimate, name)
            estimate_gradient = (-1 * u * loss_scale).detach() * self.perturbation_variance[self.current_task][name]
            if self.args.need_normalization:
    ...
    def get_parameter_value(self, model, param_name):
        ...
        #param_value = param_obj._parameters[attributes[-1]]
    ...
    def meta_perturb_parameters(self, model, scaling_factor=1):
        ...
        for name, param in self.named_parameters_to_optim:
            with torch.no_grad():
                ...
                u = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
                if self.args.need_normalization:
                    param.data = param.data + scaling_factor * u * self.perturbation_variance[self.current_task][name]  * self.normalization_factor.item() * self.args.zo_eps
                else:
                    param.data = param.data + scaling_factor * u * self.perturbation_variance[self.current_task][name] * self.args.zo_eps
    ...
    def _maybe_log_save_evaluate(self, current_loss, tr_loss, model, trial, epoch, ignore_keys_for_eval):

        if self.epoch_step%10==0: #self.control.should_log:#
            logs: Dict[str, float] = {}
            ...
            logs[f"{self.current_task}_loss "] = current_loss
            logs[f"{self.current_task} learning_rate"] = self._get_learning_rate()


            self._total_loss_scalar_dict[self.current_task] += tr_loss_scalar
            self._globalstep_last_logged = self.state.global_step
            self.store_flos()
